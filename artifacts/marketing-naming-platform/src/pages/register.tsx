...
import { useRegister, type ErrorType } from "@workspace/api-client-react";
...
export default function Register() {
  ...
  const { mutate, isPending } = useRegister({
    mutation: {
      ...
      onError(err: ErrorType<unknown>) {
        ...
        const msg = data?.error ?? err.message ?? "Registration failed";
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    mutate({ data: { teamName, email, password } });
  }
    ...
    <div className="min-h-[100dvh] flex items-center justify-center bg-background p-4">
      <Card className="w-full max-w-sm">
        <CardHeader className="text-center">
          ...
          <CardDescription>Start your team's naming convention system</CardDescription>
        </CardHeader>
        <CardContent>
